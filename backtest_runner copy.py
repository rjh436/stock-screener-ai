...
def _mindful_entry(df: pd.DataFrame, i: int) -> Optional[Dict]:
    """
    Approximate Mindful Trader entry:
    - Last 10 closes above SMA20
    - At least one Keltner upper pierce in last 10 bars
    - Entry at today's close if it is above SMA20 and below kc_upper
    """
    if i < 10:
        return None
    ...
    # Approximate projected entry as today's close
...
def _backtest_strategy(
    ...
    for current_dt in all_dates:
        ...
        for sym, pos in positions.items():
            ...
            if df_sym is None:
                continue
            if current_dt not in df_sym.index:
                continue
            i = get_row_index(df_sym, current_dt)
            if i is None:
                continue
            price = df_sym.iloc[i]["close"]
        ...
        for sym in symbols_with_data:
            ...
            df_sym = enriched[sym]
            i = get_row_index(df_sym, current_dt)
            if i is None or i < MIN_BARS_FOR_WARMUP:
                continue

            # Strategy-specific entry logic
            if strategy_name == "RHCTS":
                entry_info = _rhcts_entry(df_sym, i)
            elif strategy_name == "CRSI":
                entry_info = _crsi_entry(df_sym, i)
            elif strategy_name == "CBC":
                entry_info = _cbc_entry(df_sym, i)
            elif strategy_name == "RAPTOR":
                entry_info = _raptor_entry(df_sym, i)
            elif strategy_name == "HMP":
                entry_info = _hmp_entry(df_sym, i)
            elif strategy_name == "CGM10":
                entry_info = _cgm10_entry(df_sym, i)
            elif strategy_name == "R2X":
                entry_info = _r2x_entry(df_sym, i)
            elif strategy_name == "MINDFUL":
                entry_info = _mindful_entry(df_sym, i)
            else:
                entry_info = None
    ...
    if equity_history:
        ...
        for sym, pos in positions.items():
            df_sym = enriched.get(sym)
            if df_sym is None or last_dt not in df_sym.index:
                continue
            i = df_sym.index.get_loc(last_dt)
            price = df_sym.iloc[i]["close"]
