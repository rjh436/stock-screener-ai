...
def _compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Assumes df is already cleaned by loader.py"""
    try:
...
def run_backtest(
    ...
    if not enriched:
        return _empty_result(strategy.name, start_cash)
    ...
    for current_dt in all_dates:
        for sym in list(positions.keys()):
            ...
            if strategy.exit(df, i, pos["entry_i"], pos["entry_price"], pos["stop_price"]):
                ...
                if row["low"] < pos["stop_price"] and row["open"] < pos["stop_price"]:
                    exit_px = row["open"]
                if exit_px > (pos["entry_price"] * 4.0):
        ...
        if len(positions) < max_pos and cash > (equity * pos_pct):
