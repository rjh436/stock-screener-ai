...
def _compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        mask = df['bb_mid'] = rolling_mean
        df['bb_width'] = 0.0
        df.loc[mask != 0, 'bb_width'] = (df['bb_upper'] - df['bb_lower']) / df['bb_mid']
        
        tr = pd.concat([df['high']-df['low'], (df['high']-df['close'].shift()).abs(), (df['low']-df['close'].shift()).abs()], axis=1).max(axis=1)
        df['atr14'] = tr.rolling(14).mean()
        df['atr14_ma20'] = df['atr14'].rolling(20).mean()

        # Extremes & SHIFTED (Crucial)
...
def _empty_result(name, start_cash):
    return {
        ...
        "avg_profit_pct": 0.0, "avg_days_held": 0.0, "profit_factor": 0.0, "payoff_ratio": 0.0, "Score": 0.0
    }
