...
# Suppress benign Streamlit threading warnings
...
from execution.engine import run_backtest, run_compare
...
# Auth / Schwab wrapper
# ==============================
# SchwabData is now imported from data.schwab_client
# sd is already initialized there

# ==============================
# Index fetchers (Wikipedia) + caching
# ==============================
# Moved to data/indices.py
...
with st.sidebar:
    ...
    # Cache Management
    if st.button("🗑️ Clear Data Cache", help="Clear all cached historical data. Use if you suspect data is outdated or corrupted."):
        DataCache.clear_cache()
        st.success("Cache cleared! Next backtest will fetch fresh data.")
        st.rerun()
    ...
    strategy = st.sidebar.selectbox(
        ...
        (
            ...
            "Raptor II (Regime Mean-Reversion)",
            "High-Momentum Pullback (HMP)",
            "CGM-10 (Dual Strategy)",
            "RJH Adaptive R2-X",
            "Mindful Trader (Keltner)",
            "AI Optimized (Gen4_SMA5_Exit)",
            "AI V2 (Target or Bust)", # NEW
            "AI V3 (Deep Value Hunter)",
            "First Touch (Trend Pullback)",
            "Volatility Breakout (Trend)",
            "Apex Sniper (High Precision)",
            "Generated Strategies (Top 5)"
        )
    )
    ...
    with st.expander("📘 Strategy Rules", expanded=False):
        # RHCTS description removed as per instruction
        if strategy.startswith("ConnorsRSI") or strategy.startswith("All"):
            st.markdown("""**ConnorsRSI (Mean Reversion)**
- **Trend**: **SMA200** rising
- **Trigger**: **RSI(2) < 5** (oversold)
...
- **Risk/Targets**: Stop: **Entry - 2 * ATR(14)**, Target: **Entry + 2 * ATR(14)**, Time Stop: **9 trading days**""")
        if strategy.startswith("AI Optimized") or strategy.startswith("All"):
            st.markdown("""**AI Optimized (Gen4)**
- **Trend**: **EMA50 > EMA200** & **Close > EMA200**
- **Trigger**: **RSI(2) < 10** & **Close <= Lower BB**
- **Exit**: **Close > SMA5** (Fast Profit) or **RSI(2) > 75** (Spike)""")
        if strategy.startswith("First Touch") or strategy.startswith("All"):
            st.markdown("""**First Touch (Trend Pullback)**
- **Trend**: **EMA50 > EMA200** (Rising)
- **Catalyst**: **+20% move** in last 20 days
- **Setup**: Pullback to **EMA20/50** on low volume
- **Trigger**: Break of **Prior High**
- **Exit**: **+5% Target** (Scale Out) & **EMA10 Trail**""")
...
def _signal_rhcts(d):
    config = load_config()
    params = config.get("RHCTS", {})
    strat = StrategyRHCTS(params)
    # Strategy expects (df, i)
    res = strat.entry(d, len(d)-1)
    return "BUY" if res else None
...
def _rank_and_targets(d, mode: str):
    ...
    # Default ETA if not found
    eta = 10

    if mode == "RHCTS":
        params = config.get("RHCTS", {})
        eta = int(params.get("avg_win_duration", 10))
    ...
    if mode == "MINDFUL":
        ...
        eta = 9  # Per rules
        strength = 1.0
    elif mode == "MINDFUL": eta = int(config.get("Mindful", {}).get("avg_win_duration", 9))
    elif mode == "AI_OPT": eta = 15

    return round(close, 2), round(close + atr, 2), 2.0, eta, 1.0
...
def _signal_for_strategy(d, name: str):
    ...
    if name.startswith("RHCTS"):
        params = config.get("RHCTS", {})
        strat = StrategyRHCTS(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "RHCTS"
        
    elif name.startswith("ConnorsRSI"):
        params = config.get("ConnorsRSI", {})
        strat = StrategyConnorsRSI(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "CRSI"
        
    elif name.startswith("CBC v1"):
        params = config.get("CBC", {})
        strat = StrategyCBC(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "CBC"
        
    elif name.startswith("Raptor II"):
        params = config.get("Raptor", {})
        strat = StrategyRaptor(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "RAPTOR"
        
    elif name.startswith("High-Momentum"):
        params = config.get("HMP", {})
        strat = StrategyHMP(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "HMP"
        
    elif name.startswith("CGM-10"):
        params = config.get("CGM10", {})
        strat = StrategyCGM10(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "CGM10"
        
    elif name.startswith("RJH Adaptive"):
        params = config.get("R2X", {})
        strat = StrategyR2X(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "R2X"
        
    elif name.startswith("Mindful Trader"):
        params = config.get("Mindful", {})
        strat = StrategyMindful(params)
        sig = "BUY" if strat.entry(d, len(d)-1) else None
        return sig, "MINDFUL"
        
    elif name.startswith("AI Optimized"):
        # AI Strategy doesn't need config params yet as they are hardcoded defaults in the class
...
def schwab_data_provider_for_backtrader(symbol: str, start_date: date):
    ...
    try:
        ...
        # --- CACHE LOGIC ---
        cached_df = DataCache.get_cached_data(symbol)
        ...
        if last_cached_date:
            chunk_start = last_cached_date + timedelta(days=1)
        else:
            # Default start if no cache (e.g. 2010 to capture deep history)
        ...
        if chunk_start < now_utc:
            while chunk_start < now_utc:
                ...
                chunk_start += timedelta(days=400)
                time.sleep(0.1) # Faster rate limit since we fetch less
        ...
        if all_bars:
            ...
            if "datetime" in new_df.columns:
                new_df["datetime"] = pd.to_datetime(new_df["datetime"], unit="ms", utc=True)
        ...
        # Check if index is already a DatetimeIndex (from cache merge)
        if not isinstance(df.index, pd.DatetimeIndex):
            # Need to convert a column to index
            ...
            # Index is already DatetimeIndex, just remove duplicate indices
            df = df[~df.index.duplicated(keep='last')]
...
if "bt_start_date" not in st.session_state:
    st.session_state.bt_start_date = date(2014, 1, 1)

cols_dur = st.columns(6)
def set_date(years):
    st.session_state.bt_start_date = date.today() - timedelta(days=365*years)

with cols_dur[0]:
    if st.button("1 Year"): set_date(1)
with cols_dur[1]:
    if st.button("5 Years"): set_date(5)
with cols_dur[2]:
    if st.button("10 Years"): set_date(10)
with cols_dur[3]:
    if st.button("20 Years"): set_date(20)
with cols_dur[4]:
    if st.button("Max (30Y)"): set_date(30)

bt_start_date = st.date_input("Start Date", value=st.session_state.bt_start_date)
st.session_state.bt_start_date = bt_start_date # Sync back if user manually changes it
...
if st.button("Run Backtest", key="run_backtest_engine"):
    ...
    if not symbols:
        ...
        if not data_dict:
            ...
            strategy_names = []
            if strategy.startswith("All"):
                strategy_names = ["RHCTS", "CRSI", "CBC", "RAPTOR", "HMP", "CGM10", "R2X", "MINDFUL", "AI_OPT", "FIRST_TOUCH", "BREAKOUT", "APEX_SNIPER"]
                # Add generated strategies to "All"
                import json
                import os
                ...
                try:
                    with open(gen_config_path, "r") as f:
                        ...
                        # Limit to top 5 to avoid overcrowding
                        for g in gen_list[:5]:
                            strategy_names.append(g["name"])
                except:
                    pass
            elif strategy.startswith("ConnorsRSI"): strategy_names = ["CRSI"]
            elif strategy.startswith("CBC"): strategy_names = ["CBC"]
            elif strategy.startswith("Raptor"): strategy_names = ["RAPTOR"]
            elif strategy.startswith("High-Momentum"): strategy_names = ["HMP"]
            elif strategy.startswith("CGM-10"): strategy_names = ["CGM10"]
            elif strategy.startswith("RJH"): strategy_names = ["R2X"]
            elif strategy.startswith("Mindful"): strategy_names = ["MINDFUL"]
            elif strategy.startswith("AI Optimized"): strategy_names = ["AI_OPT"]
            elif strategy.startswith("First Touch"): strategy_names = ["FIRST_TOUCH"]
            elif strategy.startswith("Volatility Breakout"): strategy_names = ["BREAKOUT"]
            elif strategy.startswith("Apex Sniper"): strategy_names = ["APEX_SNIPER"]
            elif strategy.startswith("AI V3"): strategy_names = ["AI_V3"]
            elif strategy.startswith("Generated Strategies"):
                # Load top 5 from JSON
                import json
                import os
                gen_config_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'config/generated_strategies.json'))
                try:
                    with open(gen_config_path, "r") as f:
                        gen_list = json.load(f)
                        # Assume list is sorted by performance or just take first 5
                        strategy_names = [g["name"] for g in gen_list[:5]]
                except:
                    st.error("Could not load generated strategies.")
                    strategy_names = []

        # Fetch Global Data (SPY, VIX)
        global_data = {}
        st.write("Fetching Global Market Data (SPY, VIX)...")
        spy_df = schwab_data_provider_for_backtrader("SPY", bt_start_date)
        if spy_df is not None: global_data["SPY"] = spy_df
        
        vix_df = schwab_data_provider_for_backtrader("$VIX", bt_start_date) # Try $VIX
        if vix_df is None: vix_df = schwab_data_provider_for_backtrader("VIX", bt_start_date) # Fallback
        if vix_df is not None: global_data["VIX"] = vix_df
        ...
        try:
            with st.spinner("Running Backtrader engine... This may take several minutes."):
                ...
                if results_df is None or results_df.empty:
                    ...
                    display_df["Avg Profit %"] = results_df["avg_profit_pct"].fillna(0).round(2)
