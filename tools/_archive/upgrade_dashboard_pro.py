...
def calc_exit_plan(row, strategies_map):
    # Logic to determine the 'Target' column text
    strat_name = row['Strategy'].split(" + ")[0] # Handle merged names
    ...
    # Check for Profit Target (Gen 9 / Machine Gun)
...
# --- 2. BACKTEST ---
    ...
    if st.button("Run 5-Year Backtest"):
        with st.spinner("Running simulation..."):
            # Trigger engine directly for simplicity in this view
...
# --- 3. SIMULATOR (PRO MODE) ---
    ...
    with c1:
        if st.button("🔄 Run Daily Cycle"):
            with st.spinner("Processing..."):
                # Full Cycle: Prices -> Exits -> Entries
                trader.update_valuations() # Fills pending orders too
                ...
                # ... (Scan Logic similar to Screener but internal) ...
                # Simplified for brevity - calling existing scan structure ideal
                # Using the UI scanner result logic for consistency
                
                # (Re-implementing minimal scan loop for Simulator auto-mode)
                # In production, refactor scan logic to shared function
                pass 
    ...
    with c2:
        if st.button("📡 Refresh Prices"):
            with st.spinner("Fetching quotes..."):
                trader.update_valuations() # Just updates prices & pending fills
    ...
    if state['positions']:
        ...
        st.dataframe(
            ...
            .applymap(color_pnl, subset=["Unrealized PnL", "Return %"]),
            use_container_width=True,
            height=400
        )
    else:
