...
# --- 2. BACKTEST ---
    ...
    # Added Universe Selection for Backtest
    col_uni, col_dur = st.columns([1, 3])
    with col_uni:
        bt_universe = st.selectbox("Universe", ["S&P 500", "S&P 1500", "S&P 100"], index=1)
...
# --- 3. SUPER SIGNAL LAB ---
    ...
    if st.button("🧪 Test Super Signals"):
        with st.spinner("Running..."):
            ...
            res = run_backtest(GenericStrategy(super_conf), data)
            st.success(f"CAGR: {res['cagr']:.1%}")
            st.line_chart(res["equity_curve"])
            if res["trades_list"]:
                df_t = pd.DataFrame(res["trades_list"])
                st.dataframe(df_t)
                st.download_button("Download", df_t.to_csv(index=False).encode('utf-8'), "super_signals.csv", "text/csv")
...
# --- 4. SIMULATOR (PRO MODE) ---
    ...
    if hist:
        ...
        rate = (wins / len(df_hist)) * 100 if len(df_hist) > 0 else 0
        total_pnl = df_hist['pnl'].sum()
        
        k1, k2, k3 = st.columns(3)
        k1.metric("Realized PnL", f"${total_pnl:,.2f}", delta_color="normal")
        k2.metric("Win Rate", f"{rate:.1f}%")
