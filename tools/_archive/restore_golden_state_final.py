...
def restore_golden_state_final():
    print("🏆 EXECUTING FINAL GOLDEN STATE RESTORE (Strategies + Clamped Engine)...")

    # --- 1. RESTORE STRATEGIES (From Winning CSVs) ---
    config_path = "config/generated_strategies.json"
    strategies = [
        {
            # Source: 2025-11-30T15-21 (49% CAGR)
            ...
            "exit_rules": [],  # Let it run
            "stop_loss_atr": 4.4,
            "time_stop": 71
        },
        {
            # Source: 2025-11-30T16-34 (43% CAGR)
            ...
            "exit_rules": [
                {"type": "profit_target", "val": 1.06}  # Bank 6%
            ],
            "stop_loss_atr": 4.3,
            "time_stop": 10
        }
    ...
    print("   ✅ Config Restored: Gen12 (Long/NoTarget) + MachineGun (Short/Target).")

    # --- 2. RESTORE ENGINE (Clamped + Bifurcated) ---
...
def calculate_backtest_quality_score(row, strategy_name):
    ...
    # 2. Velocity Booster (Shared - Moderate)
    ...
    # 3. BIFURCATED TREND LOGIC
    if "MachineGun" in strategy_name:
        # Machine Gun LOVES safety. Bonus for Uptrend.
        if row.get("close", 0) > row.get("sma200", 999999):
            score += 20
    else:
        # Sniper (Gen 12) LOVES Deep Value. 
        # NO Trend Bonus. In fact, Deep Dip bonus.
        pass
    ...
    # CLAMP TO 100 (Restoring original behavior)
    # This prevents 'Falling Knives' (RSI 1) from dominating 'Quality Dips' (RSI 20)
    ...
    for current_dt in all_dates:
        ...
        if cash > 0:
            ...
            if len(positions) < max_positions:
                # Standard Sort (Score DESC). No Tie-Breaker needed if Clamped logic works as before.
...
"""
    with open(engine_path, "w") as f:
        f.write(engine_code)
    print("   ✅ Engine Restored: Clamped Scores + Bifurcated Logic.")

    print("\n🏁 RESTORE COMPLETE.")
    print("   Run Backtest to verify 49% CAGR (Sniper) + 43% CAGR (Machine Gun).")


if __name__ == "__main__":
    restore_golden_state_final()
