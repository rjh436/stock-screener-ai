...
def upgrade_backtest_suite():
    print("🚀 UPGRADING BACKTEST SUITE (Engine Data + Professional UI)...")

    # --- 1. UPGRADE ENGINE (Golden State Logic + Rich Data) ---
...
def run_backtest(strategy, data_dict, symbol_universe=None, start_cash=100000.0, start_date=None, global_data=None):
    ...
    cash = start_cash
    positions = {}
    equity_curve = [] # List of dicts for DataFrame conversion
    trade_pnls = []
    trades_list = []
    ...
    for current_dt in all_dates:
        ...
        # 3. Entries (Signal on i-1, Buy on i)
        if cash > 0:
            daily_candidates = []
...
"""
    with open(engine_path, "w") as f:
        f.write(engine_code)
    print("   ✅ Engine Upgraded: Equity Curves & Rich Data Enabled.")


    # --- 2. UPGRADE APP UI (Professional Layout) ---
    app_path = "app.py"
    app_code = """
...
"""
    with open(app_path, "w") as f:
        f.write(app_code)
    print("   ✅ App & Engine Updated: Backtest Suite is Live.")


if __name__ == "__main__":
    upgrade_backtest_suite()
