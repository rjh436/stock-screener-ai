...
def restore_clamped_golden_state():
    print("🏆 RESTORING CLAMPED GOLDEN STATE (The Validated Fix)...")

    # --- 1. RENAME PORTFOLIO (To match Engine Logic) ---
    config_path = "config/generated_strategies.json"
    portfolio = [
        {
            "name": "Apex Wealth (Gen 12)", # Matches 'Wealth' Logic (No Trend Filter)
            "type": "hybrid",
            "entry_rules": [
                {"col": "cci", "op": "<", "val": 0},
        ...
        {
            "name": "Apex Income (Gen 9)", # Matches 'Income' Logic (Trend Filter Active)
            "type": "hybrid",
            "entry_rules": [
                {"col": "close", "op": ">", "val": 0}, 
...
def calculate_backtest_quality_score(row, strategy_name):
    ...
    # 4. TREND BONUS (Targeted)
    # "Apex Income" (Gen 9) needs trend safety. 
    # "Apex Wealth" (Gen 12) ignores trend to buy crashes.
    if "Income" in strategy_name or "MachineGun" in strategy_name:
...
"""
    ...
    with open(engine_path, "w") as f:
        f.write(engine_code)
    print("   ✅ Engine Re-Clamped & Logic Synced.")


if __name__ == "__main__":
    restore_clamped_golden_state()
