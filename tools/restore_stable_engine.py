...
def calculate_backtest_quality_score(row, strategy_name):
    ...
    # 1. Base Oversold Logic (The Primary Driver)
    rsi2 = row.get("rsi2", 50)
    score += (100 - rsi2) * 2.0 
    
    # 2. Moderate Velocity Booster (Restored to +15 max)
    # The previous attempt used +50, which was too risky.
    ...
    for current_dt in all_dates:
        ...
        if cash > 0:
            ...
            if len(positions) < max_positions:
                # Restored Tie-Breaker: Score first, then deepest dip (rsi2 asc)
