...
def calculate_backtest_quality_score(row, strategy_name):
    # GOLDEN STATE HYBRID RANKING (Unclamped)
    score = 50.0
    
    # 1. Sniper Priority (The "Super Signal" Boost)
    # If it matches the Gen 12 DNA, give it a boost.
    cci = row.get("cci", 0)
...
def run_backtest(strategy, data_dict, symbol_universe=None, start_cash=100000.0, start_date=None, global_data=None):
    ...
    for current_dt in all_dates:
        ...
        # 1. Exits (Process FIRST using Today's Data)
        for sym in list(positions.keys()):
            ...
            if strategy.exit(df, i, pos["entry_i"], pos["entry_price"], pos["stop_price"]):
                ...
                # Max Gap Down Protection (e.g. 50% max loss)
        ...
        # 3. Entries (UPDATED LOGIC: Check Yesterday -> Buy Today)
        if cash > 0:
            daily_candidates = []
            ...
            for sym, df in enriched.items():
                ...
                # We need Yesterday (i-1) to have a signal
                if i < MIN_BARS + 1: continue
                
                # CHECK SIGNAL ON PREVIOUS CLOSE
                entry_signal = strategy.entry(df, i - 1)
                
                if entry_signal:
                    # SCORING based on PREVIOUS data (The data you saw when you scanned)
                    row_prev = df.iloc[i-1]
                    quality = calculate_backtest_quality_score(row_prev, strategy.name)
                    
                    # EXECUTION PRICE is TODAY'S OPEN
                    row_curr = df.iloc[i]
                    open_price = float(row_curr["open"])
                    
                    # Recalculate Stop Price relative to Entry Price?
                    # Usually stops are set relative to the setup candle (Yesterday).
                    # But if we gap up/down, we might need to adjust.
                    # For simplicity, we stick to the stop distance defined by the signal.
                    stop_distance = float(row_prev["close"]) - entry_signal["stop_price"]
                    real_stop = open_price - stop_distance
            ...
            if len(positions) < max_positions:
                # Sort: Score first, then Deepest Dip (RSI2)
