...
def calculate_backtest_quality_score(row, strategy_name):
    # UNCLAMPED BIFURCATED RANKING (The True Fix)
    score = 50.0
    
    # 1. Sniper Priority (Wealth Building)
    # Targeted Logic: Look for "Wealth" or "Gen12"
    cci = row.get("cci", 0)
    bb_width = row.get("bb_width", 0)
    
    # If it matches the Wealth DNA, boost it.
    ...
    # 3. Velocity Booster (Shared - Moderate)
    ...
    # 4. TREND BONUS (Income / Gen 9 Only)
    # Targeted Logic: Look for "Income" or "Gen9"
    if "Income" in strategy_name or "Gen9" in strategy_name or "MachineGun" in strategy_name:
        if row.get("close", 0) > row.get("sma200", 999999):
            score += 20
    else:
        # Wealth / Sniper: NO TREND BONUS. 
        # It needs to buy crashes below the SMA200.
        pass
    ...
    # CRITICAL: UNCLAMPED. Allow score to go > 100.
    ...
    for current_dt in all_dates:
        ...
        if cash > 0:
            ...
            for sym, df in enriched.items():
                ...
                # Check Previous Close (Yesterday)
